...
save_total_limit = 10
bn_freeze = int(round(5e6*(2/3))) # 2/3 of all global steps, based on Pytorch tutorial should be bigger ten qpar_freeze
...
pipe = Pipeline(Scenario.QUANT_AWARE_TUNE, model=modelQAT, dataset=train,
                training_args=training_args, metric_key_prefix="trainEuParlQAT_EuParl_eval")
...
callback2 = TestRobustCallback(pipe.trainer, validation['test'], "trainEuParlQAT_EuParl_test_cuda")
callback3 = TestRobustCallback(pipe.trainer, train['test'], "trainEuParlQAT_OpenSubs_test_cuda")
...
modelQAT.model.save_pretrained('./saved_models/trained/QAT_marian_INF1_marianmt_v2_en-sk_euparl-openSubs_model', push_to_hub=False)
