...
Trained model: QAT_marian_INF1_marianmt_v2_en-sk_euparl-openSubs_
                    model and tokenizer

metric_key_prefix format:
    trainEuParlQAT_EuParl_test_cpu
...
warmup_steps = 4000
max_steps = 1000000 # 5 million of update steps maximum, overides train_epochs...
save_total_limit = 2
...
experiment_name = "QAT train EuParl inf reset2"
...
training_args = {'output_dir':"/mnt/local/disk1/klasifikace_reflexe/MT_petrovic/in_progress/QAT_marian_INF2",
                 ...
                 'num_train_epochs': train_epochs, "max_steps": max_steps, 'predict_with_generate': True, 'generation_num_beams': 1,
...
callback1 = RobustCallback(pipe.trainer, validation['val'], "trainEuParlQAT_OpenSubs_eval")

callback2 = TestRobustCallback(pipe.trainer, train['test'], "trainEuParlQAT_EuParl_test_cuda")
callback3 = TestRobustCallback(pipe.trainer, validation['test'], "trainEuParlQAT_OpenSubs_test_cuda")
...
print("Training QAT on EuroParl:")
pipe.run()

comet_ml.get_global_experiment().set_name(experiment_name)

pipe.trainer.save_model('./saved_models/trained/QAT_marian_INF2_marianmt_v2_en-sk_euparl-openSubs_model')
...
print("Best model BLEU in-domain (EuParl) on QAT cpu")
pipeEval.run()
...
pipeEval.trainer.add_callback(CometContinueExperimentCallback())
print("Best model BLEU out-of-domain (OpenSubs) on QAT cpu")
pipeEval.run()
